...
class Rocket(implicit p: Parameters) extends CoreModule()(p)
  ...
  val id_expanded_inst = for (i <- 0 until decodeWidth)
    yield Mux(id_reg_mpc =/= UInt(0), inject_inst, ibuf.io.inst(i).bits.inst)
  val id_raw_inst = for (i <- 0 until decodeWidth)
    yield Mux(id_reg_mpc =/= UInt(0), inject_inst_raw, ibuf.io.inst(i).bits.raw)
  ...
  when (!ctrl_killd) {
    ex_ctrl := id_ctrl
    ex_reg_rvc := Mux(id_reg_mpc =/= UInt(0), Bool(false), ibuf.io.inst(0).bits.rvc)
    ex_ctrl.csr := id_csr
  ...
  val mem_nmpc = 
    (Mux(!mem_reg_ctx, SInt(0),
    Mux(mem_ctrl.mbranch && mem_br_taken, mem_reg_mpc.asSInt + ImmGen(IMM_S, mem_reg_inst),
    Mux(mem_reg_mpc === UInt(3), SInt(0), mem_reg_mpc.asSInt + SInt(1))))).asUInt
  val mem_wrong_npc =
  ...
  take_mpc_mem := mem_reg_valid && mem_wrong_nmpc && mem_reg_mcounter =/= UInt(1)
  ...
  ibuf.io.inst(0).ready := !ctrl_stalld && id_reg_mpc === UInt(0)
